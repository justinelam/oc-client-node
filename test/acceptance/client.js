...
describe('The node.js OC client', () => {
  ...
  describe('when initialised providing registries properties', () => {
      ...
      describe('when the request body is malformed (empty)', () => {
        ...
        it('should contain the error details', () => {
          ...
          const exp = getRegExpFromJson(expectedRequest),
            expected = new RegExp(
              'Error: Server-side rendering failed: request ' +
                exp +
                ' failed \\' +
    ...
    describe('when rendering component that returns headers', () => {
      let $components;
      let $errs;
      let $headers;
      before(done => {
      ...
      it('should return headers', () => {
        console.log($headers);
        expect($headers[0].header1).to.be.equal('Hello');
        expect($headers[0].header2).to.be.equal('This is a test');
      });
  ...
  describe('when correctly initialised', () => {
    ...
    describe('when server-side rendering an existing component linked to a responsive registry', () => {
      ...
      describe('when the component times-out', () => {
        ...
        const expectedRequest = {
          url:
            'http://localhost:3030/errors-component/??errorType=timeout&timeout=1000',
          method: 'get',
          headers: {
            'user-agent': 'oc-client-(.*?)',
            templates: templateHeader,
    ...
    describe('when getting components info with a non responsive registry', () => {
      ...
      const expectedError = new RegExp(
        'Getting component info failed: request ' +
          exp +
          ' failed \\(Error: connect ECONNREFUSED(.*?)\\)'
      );
